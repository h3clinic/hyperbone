"""Fox tracking report generator.

Produces fox_tracking_summary.md with full pipeline results and verdict.
"""
...
def generate_fox_tracking_report(
    stats: Dict,
    ...
    """Generate the Fox tracking validation report.

    Args:
        stats: Dict from run_proposal_skeleton.
        tracking_metrics: Dict from compute_tracking_metrics.
        output_dir: Where to write the report.
        asset_manifest: Optional asset_manifest.json contents.
        render_manifest: Optional fox_render_manifest.json contents.
    ...
    path = out / "fox_tracking_summary.md"
    ...
    if asset_manifest:
        lines.extend([
            f"- **Source**: {asset_manifest.get('source_repo', 'N/A')}",
            f"- **License**: {asset_manifest.get('license_note', 'N/A')}",
            f"- **Attribution**: {asset_manifest.get('attribution_note', 'N/A')}",
            f"- **Model path**: `{asset_manifest.get('local_model_path', 'N/A')}`",
            "",
        ])
    else:
        lines.extend([
            "- Source: KhronosGroup/glTF-Sample-Assets (Fox)",
            "- License: CC0 base model, CC-BY 4.0 rigging/animation",
            "",
        ])
    ...
    lines.extend([
        ...
        f"| Metric | Value |",
        f"|--------|-------|",
        f"| Video path | `{stats.get('video_path', 'N/A')}` |",
        f"| Frames processed | {stats.get('frames_processed', 0)} |",
        f"| Proposals generated | {stats.get('objects_proposed', 0)} |",
        f"| Graphs generated | {total} |",
        f"| **Accepted graphs** | **{accepted}** |",
        f"| Rejected graphs | {rejected} |",
        ...
        f"| Metric | Value |",
        f"|--------|-------|",
        f"| **Topology stability score** | **{topo_score:.4f}** |",
        ...
        f"| Graph presence rate | {tracking_metrics.get('graph_presence_rate', 0):.2%} |",
        "",
    ])
