"""Create Fox tracking overlay video.

Composites skeleton graphs, bboxes, and tracking info on each frame.
"""
...
def draw_overlay(frame: np.ndarray, record: dict, centroid_trail: list) -> np.ndarray:
    ...
    if len(centroid_trail) > 1:
        for i in range(1, len(centroid_trail)):
            ...
            trail_color = (int(255 * alpha), int(200 * alpha), 0)
            cv2.line(vis, pt1, pt2, trail_color, 2, cv2.LINE_AA)

    # Text overlays
    status = "ACCEPTED" if accepted else "REJECTED"
    cv2.putText(vis, f"Frame {frame_idx} | {status}", (10, 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
    cv2.putText(vis, f"Nodes: {node_count} | Edges: {edge_count}", (10, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)

    if not accepted and reject_reasons:
        reason_text = reject_reasons[0][:50]
        cv2.putText(vis, f"Reason: {reason_text}", (10, 60),
...
def make_tracking_overlay(
    ...
    for frame_idx, ts, frame in sample_frames(video_path, sample_fps):
        ...
        if frame_records:
            # Use first record for main overlay
            ...
            # Keep trail to last 30 points
            centroid_trail = centroid_trail[-30:]

            vis = draw_overlay(frame, record, centroid_trail)
        else:
            # No record for this frame — draw empty
            vis = frame.copy()
...
def main():
    parser = argparse.ArgumentParser(description="Create Fox tracking overlay video")
    parser.add_argument("--video", required=True, help="Input video path")
    parser.add_argument("--graphs", required=True, help="Path to graphs.jsonl")
    parser.add_argument("--out", required=True, help="Output overlay video path")
    parser.add_argument("--sample-fps", type=float, default=5.0)
    args = parser.parse_args()

    make_tracking_overlay(args.video, args.graphs, args.out, args.sample_fps)
